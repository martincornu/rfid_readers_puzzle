...
Adafruit_PN532 nfcArr[2] = {nfc1,nfc2};
...
void setup(void) {
  ...
  for (uint8_t i=0; i<NB_READERS;i++)
  {
    nfcArr[i].begin();
  
    uint32_t versiondata = nfcArr[i].getFirmwareVersion();
    if (! versiondata) {
      Serial.print("Didn't find PN53x board ");Serial.print(i);
      while (1); // halt
    }
...
void readTags(void)
{
  ...
  for (uint8_t i =0; i<NB_READERS;i++)
  {
    ...
    //Serial.println("Read passive tag");
    success = nfcArr[i].readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength,1000);
    ...
    if (success) {
      ...
      nfcArr[i].PrintHex(uid, uidLength);
